...
def retry_request(url: str, headers, site: str = "Tecnolife", category: str = "powerbank", max_retries: int = 1, retry_delay: int = 1):
    for i in range(max_retries):
        try:
            response = requests.get(url, headers=headers, timeout=15)
            response.raise_for_status()
            logging.info(f"✅ Connection successful ({site}) - Attempt {i+1}")
            return response
    ...
    try:
        ...
        if soup.find('script', id='__NEXT_DATA__'):
            ...
            if len(tecno_queries) < 6:
                print(f"⚠️ ساختار JSON تکنولایف تغییر کرده یا index 5 وجود ندارد ({url})")
                return [], True
...
def set_other_obj_data(other_data_obj, mobile_obj, url, category):
    ...
    known_brands = ['energizer', 'anker', 'xiaomi', 'philips', 'konfulon']
    if brand_slug.lower() in known_brands:
        ...
        if not brand:
            brand = BrandAccessories.objects.create(name_fa='نامشخص', name_en='unknown')
...
def main():
    phone_model_list = ['26_160_95/تمام-پاوربانک-ها?manufacturer_id=66_37_15_326&only_available=true']
    ...
    for phone_model in phone_model_list:
        for i in range(4):
            ...
            if not isinstance(mobile_urls, list):
                print(f"⚠️ get_mobile_info returned invalid data for {url}")
                continue
...
def accessories_powerbank_tecnolife_crawler(self):
    ...
    try:  
        ...
        if all_mobiles_objects:
            accessories_update_code_execution_state('Tecnolife', 'powerbank', True)
        else:
            accessories_update_code_execution_state('Tecnolife', 'powerbank', False, "No powerbank products found.")
        ...
        accessories_update_code_execution_state('Tecnolife', 'powerbank', False, error_message)
