                ...
                "attributes": [
                    {
                        "slug": "brand",
                        "values": [
                            "ay-my-lb", "hylw", "mybrw", "pl", "smswng",
                            "myzfyt", "khyslkht", "shyywmy", "ty-sy-ch", "hwwy"
                        ]
                    }
                ],
                "isAvailable": True,
...
def get_all_mobiles_id():
    ...
    try:
        ...
        if not res or res.status_code != 200:
            accessories_update_code_execution_state(site, 'watchs', False, f"Status code {getattr(res, 'status_code', 'None')}")
            return all_mobiles_id, getattr(res, 'status_code', 'No response')

        res = res.json()

        if res.get("errors", {}):
            accessories_update_code_execution_state(site, 'watchs', False, str(res.get("errors")))
        ...
        accessories_update_code_execution_state('Hamrahtel', 'watchs', False, error_message)
        print(error_message)
        return [], 0
...
def accessories_watchs_hamrahtel_crawler(self):
    all_mobiles: List[Dict] = []
    try:
        all_mobile_ids, _ = get_all_mobiles_id()
        if not isinstance(all_mobile_ids, list):
            accessories_update_code_execution_state('Hamrahtel', 'watchs', False, str(all_mobile_ids))
            return
        ...
        accessories_update_code_execution_state('Hamrahtel', 'watchs', True)
