        ...
        "variables": {
            ...
            "where": {
                "attributes": [
                    {
                        "slug": "brand",
                        "values": ["smswng", "shyywmy", "pl"]
                    }
                ],
                "isAvailable": True,
                "stockAvailability": "IN_STOCK",
                "price": {"range": {"gte": 0}},
                "category": {"eq": "Q2F0ZWdvcnk6MTE="},
            },
            "sortBy": {
                "channel": "customer",
                "direction": "ASC",
...
def accessories_charger_hamrahtel_crawler(self):
    ...
    try:
        ...
        if not isinstance(all_mobile_ids, list):
            error_message = str(all_mobile_ids) or "Failed to retrieve product IDs"
            accessories_update_code_execution_state('Hamrahtel', 'charger', False, error_message)
            return
        ...
        if all_mobiles:
            accessories_update_code_execution_state('Hamrahtel', 'charger', True)
        else:
            accessories_update_code_execution_state('Hamrahtel', 'charger', False, "No product data found.")

    except Exception as e:
        error_message = traceback.format_exc()
        accessories_update_code_execution_state('Hamrahtel', 'charger', False, error_message)
        print(f"❌ [Hamrahtel - charger] Crawl failed:\n{error_message}")
