...
def retry_request(url: str, headers, max_retries: int = 1, retry_delay: int = 1):
    for i in range(max_retries):
        try:
            ...
            logging.error(f"{url} - {error_message}")
            print(f"{url} - {error_message}")
            accessories_update_code_execution_state('Tecnolife', 'charger', False, error_message)
            return None
...
def set_other_obj_data(other_data_obj, mobile_obj, url, category):
    ...
    known_brands = ['apple', 'samsung', 'xiaomi']
...
def main():
    phone_model_list = ['26_158_33/شارژر?only_available=true&manufacturer_id=26_15_20']
...
def retry_main(max_retries=3, delay=5):
    ...
    if len(all_mobile_urls) == 0:
        error_message = "No mobile URLs found after maximum retries."
        accessories_update_code_execution_state('Tecnolife', 'charger', False, error_message)
...
def accessories_charger_tecnolife_crawler(self):
    ...
    try:  
        ...
        for url in all_mobile_urls:
            ...
            if not res or res.status_code != 200:
                error_message = f"Invalid response for URL: {url}"
                accessories_update_code_execution_state('Tecnolife', 'charger', False, error_message)
        ...
        if all_mobiles_objects:
            accessories_update_code_execution_state('Tecnolife', 'charger', True)
        else:
            accessories_update_code_execution_state('Tecnolife', 'charger', False, "No products found in Tecnolife crawl.")



    except Exception as e:
        error_message = traceback.format_exc()
        print(f"❌ [Tecnolife - charger] Crawl failed:\n{error_message}")
        accessories_update_code_execution_state('Tecnolife', 'charger', False, error_message)
