...
def accessories_speaker_kasra_crawler(self):
    try:
        ...
        for page_num in range(0, 3):
            response: ResponseType = retry_request(f"https://api.Kasrapars.ir/api/web/v10/product/index-brand?expand=varieties.minOrderCount%2CletMeKnowOnAvailability%2Cvarieties%2CcartFeatures%2CcoworkerShortName%2Csrc%2CisInWishList%2CpromotionCoworker%2Cdescription%2Cbrand&category_slug=speaker&page={page_num}&responseFields%5B0%5D=items", 'Kasrapars', 'speaker')
            # print(response.text)
            if not response:
                error_message = f"No response or connection failed on page {page_num}"
                accessories_update_code_execution_state('Kasrapars', 'speaker', False, error_message)
                continue

            if response.status_code != 200:
                error_message = f"HTTP {response.status_code} on page {page_num}"
                accessories_update_code_execution_state('Kasrapars', 'speaker', False, error_message)
                continue


            # print(json.loads(response.text))
            print('Accessories speaker Kasrapars')
            ...
            for item in json.loads(response.text)['items']['items']:
                ...
                for varieties in item['varieties']:
                    ...
                    for stocks in varieties['stocks']:
                        '<b>{stocks["count"]}</b>'
                        description = description + f'در انبار <b>{stocks["store"]["name"]}</b> شهر  <b>{stocks["city"]["name"]}</b> تعداد موجودی این اسپیکر <b>{stocks["count"]}</b> می باشد. <hr>'
        ...
        if all_mobile_objects:
            accessories_update_code_execution_state('Kasrapars', 'speaker', True)
        else:
            accessories_update_code_execution_state('Kasrapars', 'speaker', False, "No speaker products found.")
