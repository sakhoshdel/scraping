...
crawled_mobile_brands: List[str] = ['samsung', 'apple', 'xiaomi']
...
def accessories_charger_mobile140_crawler(self):
    try:
        ...
        for brand in crawled_mobile_brands:
            ...
            headers_category = {
                ...
                "Referer": f"https://mobile140.com/product-search/category-mobile-charger?brands={brand}&page=1",
            ...
            try:
                ...
                accessories_update_code_execution_state(SITE, 'charger', False, error_message)
                print(f"❌ [{SITE}] خطا در اتصال یا پاسخ سرور:\n{error_message}")
                continue

            except ValueError:
                error_message = traceback.format_exc()
                accessories_update_code_execution_state(SITE, 'charger', False, error_message)
                print(f"❌ [{SITE}] خطا در تبدیل JSON:\n{error_message}")
                continue


            # بررسی داده دریافتی
            if "data" not in data or not isinstance(data["data"], dict):
                error_message = "Invalid or missing 'data' key in response."
                accessories_update_code_execution_state(SITE, 'charger', False, error_message)
                print(f"❌ [{SITE}] {error_message}")
                continue
                ...
                for prodItem in dataProd["products"]['items']:
                    ...
                    for variant in data["variants"]:
                        try:
                            ...
                            error_message = traceback.format_exc()
                            accessories_update_code_execution_state(SITE, 'charger', False, error_message)
                            print(f"❌ [{SITE}] خطا در پردازش تنوع محصول:\n{error_message}")
                            continue
        ...
        if all_mobile_objects:
            accessories_update_code_execution_state(SITE, 'charger', True)
        else:
            accessories_update_code_execution_state(SITE, 'charger', False, "No product data found.")


    except Exception as e:
        error_message = traceback.format_exc()
        accessories_update_code_execution_state(SITE, 'charger', False, error_message)
        print(f"❌ [{SITE} - charger] Crawl failed:\n{error_message}")
