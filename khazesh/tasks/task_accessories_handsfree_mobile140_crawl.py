...
crawled_mobile_brands: List[str] = ['samsung', 'apple', 'anker', 'haylou', 'tch', 'qcy']
...
def retry_request(url: str, site: str, category: str, max_retries: int = 1, retry_delay: int = 1) -> ResponseType:
    for i in range(max_retries):
        try:
            ...
            logging.info(f"✅ Connection successful for {site}")
            return response
...
def accessories_handsfree_mobile140_crawler(self):
    try:
        ...
        # name_en field is equal to 'handsfree'.
        ...
        for brand in crawled_mobile_brands:
            ...
            payload_category = {
                "category": f"{brand}-handsfree",       # دسته‌بندی
            ...
            headers_category = {
                ...
                "Referer": f"https://mobile140.com/product-search/category-{brand}-handsfree?page=1",
            ...
            try:
                ...
                print(f"❌ Request error for {SITE}:\n{error_message}")
                continue
            except ValueError:
                error_message = traceback.format_exc()
                accessories_update_code_execution_state(SITE, 'handsfree', False, error_message)
                print(f"❌ JSON parse error for {SITE}:\n{error_message}")
                continue


            # بررسی داده دریافتی
            if "data" not in data or not isinstance(data["data"], dict):
                error_message = f"❌ Invalid API response structure for {SITE} (missing 'data' key)"
                accessories_update_code_execution_state(SITE, 'handsfree', False, error_message)
                print(error_message)
                continue
        ...
        if all_mobile_objects:
            accessories_update_code_execution_state(SITE, 'handsfree', True)
        else:
            accessories_update_code_execution_state(SITE, 'handsfree', False, "No handsfree products found in Mobile140 crawl.")


    except Exception as e:
        error_message = traceback.format_exc()
        accessories_update_code_execution_state(SITE, 'handsfree', False, error_message)
        print(f"❌ [{SITE} - Handsfree] Crawler failed:\n{error_message}")
