...
crawled_mobile_brands: List[str] = ['anker', 'xiaomi']
...
def retry_request(url: str, site: str, category: str, max_retries: int = 1, retry_delay: int = 1) -> ResponseType:
    HEADERS = {"From": "behnammohammadi149@gmail.cm"}
    for i in range(max_retries):
        try:
            response = requests.get(url, headers=HEADERS, timeout=15)
            response.raise_for_status()
            print(f"✅ Connection successful [{site}] - Attempt {i+1}")
            return response
...
def accessories_powerbank_mobile140_crawler(self):
    try:
        ...
        for brand in crawled_mobile_brands:
            ...
            payload_category = {
                ...
                "page": "1",                       # صفحه نتایج
                "pageSize": 24                    # تعداد آیتم‌ها در هر صفحه
            }

            headers_category = {
                "Domain": "mobile140.com",
                "Origin": "https://mobile140.com",
                "Referer": f"https://mobile140.com/product-search/category-{brand}-power-bank?page=1",
            ...
            try:
                ...
                if resProd.status_code != 200:
                    accessories_update_code_execution_state(SITE, 'powerbank', False, f"HTTP {resProd.status_code} on brand {brand}")
                    continue
                data = resProd.json()
            except ValueError:
                accessories_update_code_execution_state(SITE, 'powerbank', False, f"JSONDecodeError on brand {brand}")
                continue
            except requests.RequestException as e:
                accessories_update_code_execution_state(SITE, 'powerbank', False, f"RequestException on {brand}: {str(e)}")
                continue


            # بررسی داده دریافتی
            if not data or "data" not in data or not isinstance(data.get("data"), dict):
                print(f"❌ پاسخ نامعتبر از API برای برند {brand}")
                accessories_update_code_execution_state(SITE, 'powerbank', False, f"❌ داده نامعتبر برای برند {brand}")
                continue
                ...
                for prodItem in dataProd["products"]['items']:
                    ...
                    for variant in data["variants"]:
                        try:
                            ...
                            color_rgb = {}
                            color_str = color.get("color", "")
                            if color_str:
                                try:
                                    color_rgb = json.loads(color_str)
        ...
        if all_mobile_objects:
            accessories_update_code_execution_state(SITE, 'powerbank', True)
        else:
            accessories_update_code_execution_state(SITE, 'powerbank', False, "No powerbank products found.")


    except Exception:
        error_message = traceback.format_exc()
        accessories_update_code_execution_state(SITE, 'powerbank', False, error_message)
        print(f"❌ [Mobile140 - Powerbank] Crawler failed:\n{error_message}")
