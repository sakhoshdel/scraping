...
def retry_request(url: str, max_retries: int = 3, retry_delay: int = 1) -> ResponseType:
    for i in range(max_retries):
        try:
            ...
            logging.error(f"{url} - {error_message}")
            print(f"{url} - {error_message}")
            update_code_execution_state('Diarhamrah', False, error_message)
    ...
    try:
        ...
        for brand_fa in crowled_mobile_brands:
            ...
            for page_num in range(1, 4):  # Crawling first 3 pages
                ...
                category_url =f"https://diarhamrah.com/search/{brand_fa}?page={page_num}"
                print(category_url)
        ...
        update_code_execution_state('Diarhamrah', True)
