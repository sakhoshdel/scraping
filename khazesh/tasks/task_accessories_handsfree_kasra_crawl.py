...
def accessories_handsfree_kasra_crawler(self):
    try:
        ...
        for page_num in range(0, 3):
            response: ResponseType = retry_request(f"https://api.Kasrapars.ir/api/web/v10/product/index-brand?expand=varieties.minOrderCount%2CletMeKnowOnAvailability%2Cvarieties%2CcartFeatures%2CcoworkerShortName%2Csrc%2CisInWishList%2CpromotionCoworker%2Cdescription%2Cbrand&page={page_num}&responseFields%5B0%5D=items&category_slug=bluetooth-handsfree&status_available=1&brand_slug[]=samsung&brand_slug[]=apple&brand_slug[]=tch&brand_slug[]=anker&brand_slug[]=qcy&brand_slug[]=haylou", 'Kasrapars', 'handsfree')
            # print(response.text)
            if not response or response.status_code != 200:
                error_message = f"Invalid response from Kasrapars (status={getattr(response, 'status_code', 'None')})"
                accessories_update_code_execution_state('Kasrapars', 'handsfree', False, error_message)
                print(f"❌ {error_message}")
                continue


            # print(json.loads(response.text))
            print('Accessories handsfree Kasrapars')
            ...
            try:
                ...
                error_message = traceback.format_exc()
                accessories_update_code_execution_state('Kasrapars', 'handsfree', False, error_message)
                print(f"❌ Failed to parse Kasrapars JSON:\n{error_message}")
                continue
            ...
            for item in items:
                ...
                for varieties in item['varieties']:
                    ...
                    for stocks in varieties['stocks']:
                        '<b>{stocks["count"]}</b>'
                        description = description + f'در انبار <b>{stocks["store"]["name"]}</b> شهر  <b>{stocks["city"]["name"]}</b> تعداد موجودی این هندزفری <b>{stocks["count"]}</b> می باشد. <hr>'
        ...
        if all_mobile_objects:
            accessories_update_code_execution_state('Kasrapars', 'handsfree', True)
        else:
            accessories_update_code_execution_state('Kasrapars', 'handsfree', False, "No handsfree products found in Kasrapars crawl.")


    except Exception as e:
        error_message = traceback.format_exc()
        accessories_update_code_execution_state('Kasrapars', 'handsfree', False, error_message)
        print(f"❌ [Kasrapars - Handsfree] Crawler failed:\n{error_message}")
