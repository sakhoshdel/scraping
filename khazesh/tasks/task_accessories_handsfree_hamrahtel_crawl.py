        ...
        "variables": {
            ...
            "where": {
                "attributes": [
                    {
                        "slug": "brand",
                        "values": ["pl", "smswng", "nkhr", "hwwy", "hylw", "ty-sy-ch", "khyw-sy-wy"]
                    }
                ],
                "isAvailable": True,
                "stockAvailability": "IN_STOCK",
                "price": {"range": {"gte": 0}},
                "category": {"eq": "Q2F0ZWdvcnk6Nw=="},
            },
            "sortBy": {
                "channel": "customer",
                "direction": "ASC",
...
def retry_request(
    ...
    for i in range(max_retries):
        try:
            ...
            accessories_update_code_execution_state(site, 'handsfree', False, error_message)
            print(f"❌ [{site}-handsfree] Request error:\n{error_message}")
            if i < max_retries - 1:
                print("Retrying...")
                time.sleep(retry_delay)
    else:
...
def get_all_mobiles_id():
    ...
    if res is None or res.status_code != 200:
        error_message = f"Failed to fetch product list from {site}, status: {getattr(res, 'status_code', 'None')}"
        accessories_update_code_execution_state(site, 'handsfree', False, error_message)
    ...
    res = res.json()
    if res.get("errors"):
        error_message = f"GraphQL returned errors: {res.get('errors')}"
        accessories_update_code_execution_state(site, 'handsfree', False, error_message)
        print(f"❌ {error_message}")
        return [], 0
...
def get_mobile_data(mobile_id: Union[int, str]) -> Dict:
    ...
    try:
        res: ResponseType = retry_request(url, site=site, post_data=body_for_single_mobile_data, req_type="post")
        if not res or res.status_code != 200:
            error_message = f"Bad response for mobile {mobile_id} (status={getattr(res, 'status_code', 'None')})"
            accessories_update_code_execution_state(site, 'handsfree', False, error_message)
            print(f"❌ {error_message}")
            return []
        ...
        res = res.json()
        if res.get("errors"):
            error_message = f"GraphQL error for mobile {mobile_id}: {res.get('errors')}"
            accessories_update_code_execution_state(site, 'handsfree', False, error_message)
            print(f"❌ {error_message}")
            return []
        ...
        if not mobile_detail:
            error_message = f"Empty product detail for {mobile_id}"
            accessories_update_code_execution_state(site, 'handsfree', False, error_message)
            print(f"❌ {error_message}")
            return []
        ...
        error_message = traceback.format_exc()
        accessories_update_code_execution_state(site, 'handsfree', False, error_message)
        print(f"❌ [{site}-handsfree] get_mobile_data failed:\n{error_message}")
        return []
...
def accessories_handsfree_hamrahtel_crawler(self):
    ...
    try:
        ...
        if not isinstance(all_mobile_ids, list):
            error_message = f"Invalid response from get_all_mobiles_id: {all_mobile_ids}"
            accessories_update_code_execution_state('Hamrahtel', 'handsfree', False, error_message)
            print(f"❌ {error_message}")
            return
        ...
        if all_mobiles:
            accessories_update_code_execution_state('Hamrahtel', 'handsfree', True)
        else:
            accessories_update_code_execution_state('Hamrahtel', 'handsfree', False, "No handsfree products found.")

    except Exception as e:
        error_message = traceback.format_exc()
        accessories_update_code_execution_state('Hamrahtel', 'handsfree', False, error_message)
        print(f"❌ [Hamrahtel-handsfree] Crawl failed:\n{error_message}")
