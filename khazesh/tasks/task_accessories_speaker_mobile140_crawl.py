...
crawled_mobile_brands: List[str] = ['jbl', 'xiaomi', 'harman-kardon', 'anker']
...
def retry_request(url: str, max_retries: int = 1, retry_delay: int = 1) -> ResponseType:
    for i in range(max_retries):
        try:
            ...
            accessories_update_code_execution_state(SITE, 'speaker', False, f"Connection error on attempt {i+1}: {ce}")
            ...
            accessories_update_code_execution_state(SITE, 'speaker', False, f"Request error on attempt {i+1}: {re}")
            if i < max_retries - 1:
                time.sleep(retry_delay)
                continue
            return None
...
def accessories_speaker_mobile140_crawler(self):
    try:
        ...
        for brand in crawled_mobile_brands:
            ...
            headers_category = {
                ...
                "Referer": f"https://mobile140.com/product-search/category-{brand}-speaker?page=1",
            ...
            try:
                ...
                error_message = f"❌ خطا در اتصال به سرور Mobile140 (ProductSearch): {str(e)}"
                print(error_message)
                accessories_update_code_execution_state(SITE, 'speaker', False, error_message)
                continue
            except ValueError:
                accessories_update_code_execution_state(SITE, 'speaker', False, '❌ خطا در تبدیل پاسخ ProductSearch به JSON')
            ...
            if "data" not in data or not isinstance(data["data"], dict):
                ...
                accessories_update_code_execution_state(SITE, 'speaker', False, "❌ کلید 'data' در پاسخ موجود نیست یا ساختار نادرست دارد.")
                continue
            ...
            if True:
                for prodItem in dataProd["products"]['items']:
                    ...
                    try:
                        ...
                        error_message = f"❌ خطا در Product/Preview برای {slug}: {str(e)}"
                        accessories_update_code_execution_state(SITE, 'speaker', False, error_message)
                        continue

                    except ValueError:
                        print("❌ خطا در تبدیل پاسخ به JSON")
                        accessories_update_code_execution_state(SITE, 'speaker', False, f"❌ خطا در JSON Product/Preview برای {slug}")
        ...
        if all_mobile_objects:
            ...
            accessories_update_code_execution_state(SITE, 'speaker', False, "No speaker products found.")
