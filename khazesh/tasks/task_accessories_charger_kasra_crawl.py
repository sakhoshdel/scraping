    ...
    try:
        ...
        for page_num in range(0, 3):
            ...
            if not response or response.status_code != 200:
                error_message = f"Invalid response or status code: {getattr(response, 'status_code', 'None')}"
        ...
        if all_mobile_objects:
            ...
            accessories_update_code_execution_state('Kasrapars', 'charger', False, "No products found in crawl.")


    except Exception as e:
        error_message = traceback.format_exc()
        accessories_update_code_execution_state('Kasrapars', 'charger', False, error_message)
        print(f"❌ [Kasrapars - charger] Crawl failed:\n{error_message}")
