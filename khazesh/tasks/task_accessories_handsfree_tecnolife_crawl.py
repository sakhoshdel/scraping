    ...
    for i in range(max_retries):
        try:
            ...
            if i < max_retries - 1:
                ...
                accessories_update_code_execution_state('Tecnolife', 'handsfree', False, error_message)
                return None
...
def get_mobile_info(headers, url):
    try:
        ...
        if not res or res.status_code != 200:
            accessories_update_code_execution_state(
                'Tecnolife', 'handsfree', False,
                f"Bad response ({getattr(res, 'status_code', 'None')}) from {url}"
            )
            return [], True
        ...
        if not script_tag:
            print("⚠️ No NEXT_DATA script found.")
            return [], True

        tenco_page_obj = json.loads(script_tag.string.encode().decode())
        tecno_queries = (
            tenco_page_obj.get('props', {})
            .get('pageProps', {})
            .get('dehydratedState', {})
            .get('queries', [])
        )
        ...
        if len(tecno_queries) < 6:
            print("⚠️ Unexpected query index in Tecnolife structure.")
            return [], True
...
def set_other_obj_data(other_data_obj, mobile_obj, url, category):
    ...
    known_brands = ['apple', 'samsung', 'huawei', 'anker', 'haylou', 'TCH', 'QCY']
...
def main():
    phone_model_list = ['31_157_751/هندزفری-هدفون-هدست?manufacturer_id=20_26_23_37_44_546_70&only_available=true']
...
def retry_main(max_retries=3, delay=5):
    ...
    if len(all_mobile_urls) == 0:
        error_message = "No mobile URLs found after maximum retries in Tecnolife handsfree"
        accessories_update_code_execution_state('Tecnolife', 'handsfree', False, error_message)
...
def accessories_handsfree_tecnolife_crawler(self):
    ...
    try:  
        ...
        for url in all_mobile_urls:
            ...
            res = retry_request(url, headers=HEADERS)
            if not res or res.status_code != 200:
                accessories_update_code_execution_state('Tecnolife', 'handsfree', False, f"Failed to fetch product page: {url}")
                continue
        ...
        if all_mobiles_objects:
            accessories_update_code_execution_state('Tecnolife', 'handsfree', True)
        else:
            accessories_update_code_execution_state('Tecnolife', 'handsfree', False, "No handsfree products found in Tecnolife crawl.")



    except Exception:
        error_message = traceback.format_exc()
        print(f"❌ [Tecnolife - Handsfree] Crawler failed:\n{error_message}")
        accessories_update_code_execution_state('Tecnolife', 'handsfree', False, error_message)
