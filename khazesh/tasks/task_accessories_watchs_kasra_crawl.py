...
def accessories_watchs_kasra_crawler(self):
    try:
        ...
        for page_num in range(0, 3):
            response: ResponseType = retry_request(f"https://api.Kasrapars.ir/api/web/v10/product/index-brand?expand=varieties.minOrderCount%2CletMeKnowOnAvailability%2Cvarieties%2CcartFeatures%2CcoworkerShortName%2Csrc%2CisInWishList%2CpromotionCoworker%2Cdescription%2Cbrand&category_slug=watchs&page={page_num}&responseFields%5B0%5D=items&status_available=1&brand_slug[]=samsung&brand_slug[]=apple&brand_slug[]=xiaomi&brand_slug[]=tch&brand_slug[]=huawei&brand_slug[]=mibro&brand_slug[]=haylou", 'Kasrapars', 'watchs')
            # print(response.text)
            if not response:
            ...
            for item in json.loads(response.text)['items']['items']:
                ...
                for varieties in item['varieties']:
                    ...
                    for stocks in varieties['stocks']:
                        '<b>{stocks["count"]}</b>'
                        description = description + f'در انبار <b>{stocks["store"]["name"]}</b> شهر  <b>{stocks["city"]["name"]}</b> تعداد موجودی این ساعت <b>{stocks["count"]}</b> می باشد. <hr>'
